...
them 
        ...
        // If no two elements were swapped by inner loop, then the 
...
void quickSort(int arr[], int low, int high) { 
    if (low < high) { 
        ...
        quickSort(arr, low, pi - 1);  // Sort elements before the 
pivot 
...
//Practical No.: 2 
//Program Description: Implementation of Binary Search on a list of numbers stored in an 
//Array 
//Solution: 
#include <iostream> 
using namespace std; 
...
"kiwi", "mango", "orange", "pear", "watermelon"}; 
    int size = sizeof(arr) / sizeof(arr[0]); 
